                        ...
                        <tbody>
                            <tr>
                                <td style="text-align:center;padding: 0px 30px 0px 20px">
                                    ...
                                    <p style="text-align:left;color: #000000; font-size: 14px;">Tolong berikan persetujuan untuk Izin Lokasi Lahan dengan detail :</p>
                                    ...
                                    <b style="font-style:italic;">Untuk melihat lampiran, tolong klik tautan dibawah ini : </b><br>
                                        @if ( is_array($data['url_link']) || is_object($data['url_link']) )
                                            @foreach ($data['url_link'] as $tampil)
                                                <a href={{ str_replace(' ', '%20', $tampil) }} target="_blank">{{ trim(str_replace('%20', ' ',substr($tampil, strrpos($tampil, '/') + 1))) }}</a><br><br>
                                            @endforeach
                                        @else
                                            <a href={{ str_replace(' ', '%20', $data['url_link']) }} target="_blank">{{ trim(str_replace('%20', ' ',substr($data['url_link'], strrpos($data['url_link'], '/') + 1))) }}</a><br><br>
                                        @endif
                                    </p>
                                    @endif
                                </td>
