                        ...
                        <tbody>
                            <tr>
                                <td style="text-align:center;padding: 30px 30px 20px">
                                    ...
                                    <p style="margin-bottom: 15px; color: #526484; font-size: 16px;">Tolong berikan persetujuan untuk Gambar Ukur yang terbentuk setelah dilakukan pengukuran, dengan nomor dokumen {{ $data['doc_no'] }},NOP: {{ $data['nop_no'] }}, Nama Pemilik: {{ $data['name_owner'] }}, dengan luas tanah Gambar Ukur sebesar {{ $data['land_area'] }}m&sup2;.</p>
                                        ...
                                        @if ($data['url_link'] != 'EMPTY')
                                            @if ( is_array($data['url_link']) || is_object($data['url_link']) )
                                                @foreach ($data['url_link'] as $tampil)
                                                    <a href={{ $tampil }} target="_blank">{{ trim(substr($tampil, strrpos($tampil, '/') + 1)) }}</a><br><br>
                                                @endforeach
                                            @else
                                                <a href={{ $data['url_link'] }} target="_blank">{{ trim(substr($data['url_link'], strrpos($data['url_link'], '/') + 1)) }}</a><br><br>
                                            @endif
                                        @endif
                                    </p>
                                </td>
