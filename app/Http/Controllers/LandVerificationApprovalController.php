...
class LandVerificationApprovalController extends Controller
{
    ...
    public function changestatus($status='', $entity_cd='', $doc_no='', $level_no='')
    {
        ...
        if(count($query)>0){
            ...
            if($status == 'A') {
                $sqlsendemail = "mgr.xrl_send_mail_approval_land_verification '" . $entity_cd . "', '" . $doc_no . "', '" . $status . "', '" . $level_no . "'";
                ...
                $snd = DB::connection('SSI')->insert($sqlsendemail);
                if ($snd == '1') {
                    $msg = "You Have Successfully Canceled the Verification Document No. ".$doc_no;
                    $notif = 'Canceled !';
                    $st = 'OK';
                    $image = "reject.png";
                } else {
